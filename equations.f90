    ...
    bbar  = Ini_Read_Double('bbar',0.0d0)
    ...
    w_de=(cs2_lam*bbar*a**(-4.0d0)-1.0d0)/(bbar*a**(-4.0d0)+1.0d0)
    ...
    use constants  
    real(dl) :: dwdlna_de
    real(dl) :: rho_0, rho_ae, drho_ae, dw_ae, w_ae
    real(dl), intent(IN) :: a
    ...
    ! grho_de=grhov*(a**4.0d0 + as/w1 *deq/(1.0d0 -deq))
    grho_de=grhov/(bbar+1)*(a**4.0d0+bbar)
    ...
    ! Christiane 
    !open(20,file="test.dat",position="append")
    !write(20,*) a, w_de(a), grho_de(a)/grhoa2 
    !close(20) 
    ...
    ISW = (4.D0/3.D0*k*EV%Kf(1)*sigma+(-2.D0/3.D0*sigma-2.D0/3.D0*etak/adotoa)*k &
     ...
     !open(20,file='ISW1.dat',position='append')
     !write(20,*) a, ISW 
     ! close(20) 
